...
(function (NSCode) {
  ...
  NSCode.pageNotes = {
    ask:
      '本物の LLM（Claude）と同じ<b>「検索＋重み」</b>のハイブリッドで回答するページ。質問のたびに、まず知識ベースから<b>関連箇所を検索</b>し、その文脈を<b>極小ニューラルネット（埋め込み→隠れ層→softmax）が学習</b>して、<b>重みから回答を生成</b>します。関連部分だけを学習するので、大きな PDF/資料でもスケールします。回答の下に検索した根拠（参照箇所）も表示。<b>赤ちゃん級</b>のため回答は不完全なことがあります（重みは Neural Lab、PDF取り込みは PDF抽出 で）。既定の知識はプラントエンジニア向け日本語サンプル＋廃棄物発電施設の解説です。',
    howto:
