...
(function (NSCode) {
  ...
  NSCode.pageNotes = {
    ask:
      '自分の文書を入れて質問すると、<b>その文書だけを根拠に</b>回答する RAG ページ。文書をチャンク分割 → TF-IDF コサインで関連箇所を検索 → 重複を避けて要点文を選抜し、<b>出典付き</b>で回答します。検索は本物の語彙ベース、回答は抽出型（生成なし）。仕組みは「How To Answer」で詳しく解説。',
    howto:
    ...
    'claude-code':
      'Claude Code のアーキテクチャを学ぶ教育ページ。while ループ / 権限ゲート / セッション / メモリ、最後に最小ハーネスのブラウザ実行。公開分析に基づく概念モデル（数値は概数）。',
    'ai-coding':
