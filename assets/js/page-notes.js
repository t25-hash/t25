...
(function (NSCode) {
  ...
  NSCode.pageNotes = {
    ask:
      '文書を<b>ニューラルネットに学習させ、その重み（記憶）から質問に回答</b>するページ。<b>RAG（文書検索）ではありません</b>——ナレッジベースの内容を極小ニューラルネット（埋め込み→隠れ層→softmax）に勾配降下で取り込み、質問のキーワードを起点に学習済みの重みから回答を生成します。文書を追加・削除すると再学習し、追加してもしなくても重みの中の知識から答えます。<b>赤ちゃん級</b>のため回答は不完全なことがあります（中身は Neural Lab で観察可）。既定の学習データはプラントエンジニア向け日本語サンプル＋廃棄物発電施設の解説です。',
    howto:
