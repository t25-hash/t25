...
(function (NSCode) {
  ...
  NSCode.pageNotes = {
    ask:
      '自分の文書を入れて質問すると、RAG の手順（<b>検索 → 根拠から回答を構成</b>）で答える教育ページ。関連チャンクを TF-IDF で検索し、その中から質問に最も関連する文を抽出して<b>自然な日本語の回答</b>を組み立てます（根拠も併記）。あわせて、ブラウザ内の<b>極小LLM（n-gram・赤ちゃん級・API不要）</b>が1トークンずつ「生成」する様子も学習用に表示します。既定のナレッジベースは<b>プラントエンジニア向けの日本語サンプル</b>で、本プラットフォームの全機能（RAG / Embedding / Agent / MCP …）をプラント業務の文脈で解説しています。仕組みは「How To Answer」で詳しく解説。',
    howto:
