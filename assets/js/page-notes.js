...
(function (NSCode) {
  ...
  NSCode.pageNotes = {
    ask:
      '自分の文書を入れて質問すると、Claude Code の手順どおり（検索 → 文脈で学習 → 生成）に答える教育ページ。関連チャンクを TF-IDF で検索し、その文脈で<b>ブラウザ内の極小LLM（n-gram・赤ちゃん級・API不要）</b>が<b>次トークン予測で回答を生成</b>します。生成過程（候補と確率）も表示。既定のナレッジベースは<b>プラントエンジニア向けの日本語サンプル</b>で、本プラットフォームの全機能（RAG / Embedding / Agent / MCP …）をプラント業務の文脈で解説しています。仕組みは「How To Answer」で詳しく解説。',
    howto:
