...
(function (NSCode) {
  ...
  var SYSTEMS = [
    { id: 'Context System', tag: 'infra', desc:
      ...
      links: [{ t: 'Ask (RAG)', r: '#/ask' }, { t: 'RAG Lab', r: '#/rag' }] },
    { id: 'Permission System', tag: 'infra', desc:
      ...
      links: [{ t: 'Tool Calling Lab', r: '#/tools' }, { t: 'Mini Harness（権限ゲート）', r: '#/claude-code/harness' }] },
    { id: 'Tool System', tag: 'infra', desc:
      ...
      links: [{ t: 'Tool Calling Lab', r: '#/tools' }, { t: 'MCP Lab', r: '#/mcp' }] },
    ...
    { id: 'SubAgent', tag: 'infra', desc:
      ...
      links: [{ t: 'Multi-Agent Lab', r: '#/multi-agent' }] }
  ];
  ...
  function mountArchitecture() {
    ...
    function renderDetail() {
      ...
      var links = (s.links || []).map(function (l) {
        return '<a class="ns-btn ns-btn--ghost" href="' + l.r + '">' + C.esc(l.t) + ' →</a>';
      }).join('');
      out.innerHTML =
        '<div class="cc-detail">' +
          ...
          (links ? '<div class="cc-detail__links"><span class="ns-empty__hint">対応するページで体験 → </span>' + links + '</div>' : '') +
        '</div>';
    }
    renderGrid(); renderDetail();
  }
