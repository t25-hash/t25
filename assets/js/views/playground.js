/* Playground (PLAY) — LLM & Prompt experimentation, offline.
...
(function (NSCode) {
  ...
  function header(s) { return C.PageHeader({ title: s.title, purpose: s.purpose, breadcrumb: ['Playground', s.title] }) + C.Tabs(tabs, s.route); }
  function tokCount(t) { return T.tokenize(t || '').length; }

  /* ---------- LLM Playground ---------- */
  NSCode.registerView({
    route: '#/playground/llm', module: 'playground', title: 'LLM Playground',
    render: function () {
      var opts = MODELS.map(function (m) { return '<option' + (m === state.model ? ' selected' : '') + '>' + m + '</option>'; }).join('');
      return header({ title: 'LLM Playground', purpose: 'モデル・パラメータを変えて実験（応答はシミュレーション）', route: '#/playground/llm' }) +
        C.Panel({ title: '設定', hint: 'トークン数は実カウント / 応答はオフライン擬似生成', body: C.Controls([
        ...
        C.Panel({ title: 'プロンプト', body: '<textarea id="pgPrompt" class="ns-input" rows="4">' + C.esc(state.prompt) + '</textarea>' +
          '<div class="ns-actions"><button id="pgRun" class="ns-btn">実行</button></div>' }) +
        C.Panel({ title: 'リクエスト / トークン', hint: 'API へ送られる構造（実カウント）', body: '<div id="pgReq"></div>' }) +
        C.Panel({ title: '応答（シミュレーション）', hint: 'backend 未接続: 実モデル応答には API 接続が必要 (docs/05)', body: '<div id="pgResp"></div>' }) +
        C.Panel({ title: 'モデル別トークン/コスト試算', hint: '同一プロンプトでの概算（料金は例示値）', body: '<div id="pgCompare"></div>' });
    },
    onMount: function () {
  ...
  function renderLLM() {
    ...
    var reqObj = {
      model: state.model, temperature: state.temp,
    ...
    var rows = MODELS.map(function (m) {
      return [m, String(total), '$' + (total / 1000 * PRICE[m]).toFixed(5)];
    });
  ...
  NSCode.registerView({
    route: '#/playground/prompt', module: 'playground', title: 'Prompt Playground',
    render: function () {
      return header({ title: 'Prompt Playground', purpose: 'プロンプトの品質をヒューリスティック評価・改善', route: '#/playground/prompt' }) +
        C.Panel({ title: 'プロンプト', body: '<textarea id="evPrompt" class="ns-input" rows="4">' + C.esc(state.promptEval) + '</textarea>' +
          '<div class="ns-actions"><button id="evImprove" class="ns-btn">改善案を生成</button></div>' }) +
        C.Panel({ title: '評価', hint: '6観点のチェック（ルールベース）', body: '<div id="evScore"></div>' }) +
        C.Panel({ title: '改善案 (Before / After)', body: '<div id="evImproved"></div>' });
    },
    onMount: function () {
  ...
  function renderEval() {
    ...
    out.innerHTML =
      C.ProgressBar({ label: '総合スコア', percent: score }) +
