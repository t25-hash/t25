...
> ⚠️ **前提: 外部 AI API は一切使いません。** LLM/Embedding/要約/QA などの生成系は、
> 外部プロバイダ（Claude / OpenAI 等）の API を呼ばず、すべて**ブラウザ内エンジン**
> （`llm-engine.js` / `embed-engine.js` / `rag-engine.js` / `neural-engine.js` ほか）で
> 完結します。本書のバックエンド API は、進捗・成果物・設定などの**永続化のみ**を担い、
> 生成系のエンドポイントを実装する場合も**同じローカルロジックをサーバ側で動かす**もので、
> 外部 AI への中継はしません。**API キーは不要**です。
...
- エラー: 下記共通フォーマット。
- 生成系（LLM/Embedding 等）は **ブラウザ内エンジンで完結**。外部 AI プロバイダの
  API は呼ばず、API キーも持たない。
...
> **実装メモ**: Playground / 生成系は **ブラウザ内エンジンで実動**します（`llm-engine.js`）。
> 外部 AI API は呼びません。`model` はローカルエンジンの識別子（例 `sml-v0`）で、
> `usage` は外部課金ではなくローカル処理量の目安です。
...
> **実装メモ（雛形）**: Embedding Lab は **完全クライアントサイドで実動**します（`embed-engine.js`）。
> 埋め込みは **signed hashing trick**（語彙ベース・決定論的、学習済みニューラル埋め込みではない）、
> 類似度は cos/euclid/dot、クラスタは **PCA(2D)**（べき乗法）。将来 dense embedding や
> UMAP/3D 可視化へ高度化する場合も、**ローカル実装**を差し替えます（外部 AI API は使いません）。
...
> **実装メモ（雛形）**: RAG Lab は **完全クライアントサイドで実動**します（`rag-engine.js`）。
> Chunk（文字窓 / セパレータ対応 + オーバーラップ）、Retrieval（**TF-IDF コサイン** = 語彙ベース。
> dense embedding ではない）、ReRank（**MMR** で関連性×多様性）、Context Builder（テンプレート差込）、
> Hallucination（回答×コンテキストの**語彙重なり**による簡易検出）。高品質化する場合も、
> 検索・要約・判定は**ローカルエンジンの強化**で行います（外部 LLM API には依存しません）。
...
> PDF 解析（テキスト抽出・ページ描画）は同梱の **pdf.js**。`research-engine.js` が
> オフラインで以下を提供します（外部送信なし）:
> - **要約**: TextRank（文類似度グラフ + PageRank、`NSCode.embeddings` 利用）
> - **キーワード**: TF-IDF（ページを文書として算出）
> - **抽出型 QA**: `NSCode.rag` で関連パッセージを検索し該当文を抜粋（生成なし）
> - **文書内検索**: 語を含む文をページ番号付きで抽出
>
> 高品質化する場合も**ローカルの要約/抽出ロジックの強化**で対応します（外部 AI API は使いません）。
> フロントの `NSCode.api.savePaper / listPapers` は localStorage に保存します（docs/04 papers 参照）。
...
- 重い処理（生成系のローカル実行）が輻輳した場合は 429 + `Retry-After`。フロントは指数バックオフ。
- 外部 AI API を使わないため**従量課金は発生しません**。`usage`（トークン数）はローカル処理量の
  目安として返し、Evaluation/Playground で表示します（`cost` は概算/常に 0 でも可）。
