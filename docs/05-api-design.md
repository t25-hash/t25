...
> 類似度は cos/euclid/dot、クラスタは **PCA(2D)**（べき乗法）。上記 API は dense embedding
> モデル（例: 埋め込みAPI）や UMAP/3D 可視化へ差し替える際の接続先です。
...
> Hallucination（回答×コンテキストの**語彙重なり**による簡易検出）。上記 API は、検索を
> dense embedding に、要約/判定を LLM に置き換えて高品質化する際の接続先です。
...
>
> 上記 API はサーバ側 **LLM 要約/生成 QA**（高品質）へ拡張する際の接続先です。フロントの
